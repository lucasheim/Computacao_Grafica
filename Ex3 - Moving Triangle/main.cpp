...
int main() {
	...
	*/
	...
	float xSpeed = 1.0f;
	float ySpeed = 0.7f;
	float xLastPosition = 0.0f;
	float yLastPosition = 0.0f;
	glUseProgram(shader_programme);
	while (!glfwWindowShouldClose(window)) {
		...
		if (elapsedSeconds > 0) {
			...
			if (fabs(xLastPosition) + 0.5f > 1.0f) {
				normal = xLastPosition > 0 ? 1 : -1;
				xSpeed = 2 * normal * (normal * -xSpeed) + xSpeed;
			}
			
			if (fabs(yLastPosition) + 0.5f > 1.0f) {
				normal = yLastPosition > 0 ? 1 : -1;
				ySpeed = 2 * normal * (normal * -ySpeed) + ySpeed;
			}
